...
import { getApiErrorMessage, useUnlockPdfMutation } from "../hooks/usePdfMutations";

interface UnlockPdfPageProps {
  pageCopy: ToolPageCopy;
}

export default function UnlockPdfPage({ pageCopy }: UnlockPdfPageProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!password) { setError("Please enter the PDF's password."); return; }

    try {
      const result = await unlockMutation.mutateAsync({ file, password });
      setMessage(result.message);
      setDownloadUrl(getDownloadUrl(result.download_url));
    } catch (err) {
      setError(getApiErrorMessage(err, "Unlock failed."));
    }
  };
      ...
      <section className="app-shell tool-page-shell relative">
        ...
        <div className="glass-card tool-page-panel">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button onClick={handleSubmit} disabled={unlockMutation.isPending || !file || !password} className="btn-accent">
              {unlockMutation.isPending ? pageCopy.actions.busyLabel : pageCopy.actions.primaryLabel}
            </button>
          </div>
