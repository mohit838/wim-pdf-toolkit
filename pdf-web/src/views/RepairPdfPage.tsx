...
import { getApiErrorMessage, useRepairPdfMutation } from "../hooks/usePdfMutations";

interface RepairPdfPageProps {
  pageCopy: ToolPageCopy;
}

export default function RepairPdfPage({ pageCopy }: RepairPdfPageProps) {
  const fileInputCopy = pageCopy.fileInput!;
  const [file, setFile] = useState<File | null>(null);
  const [message, setMessage] = useState("");
  const [downloadUrl, setDownloadUrl] = useState("");
  const [error, setError] = useState("");
  const mutation = useRepairPdfMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Repair failed."));
    }
  };
