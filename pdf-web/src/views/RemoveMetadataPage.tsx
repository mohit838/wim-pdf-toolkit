...
import { getApiErrorMessage, useRemoveMetadataMutation } from "../hooks/usePdfMutations";

interface RemoveMetadataPageProps {
  pageCopy: ToolPageCopy;
}

export default function RemoveMetadataPage({ pageCopy }: RemoveMetadataPageProps) {
  const fileInputCopy = pageCopy.fileInput!;
  const [file, setFile] = useState<File | null>(null);
  const [message, setMessage] = useState("");
  const [downloadUrl, setDownloadUrl] = useState("");
  const [error, setError] = useState("");
  const mutation = useRemoveMetadataMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Removing metadata failed."));
    }
  };
