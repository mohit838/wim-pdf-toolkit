...
import { getApiErrorMessage, usePdfToTextMutation } from "../hooks/usePdfMutations";

interface PdfToTextPageProps {
  pageCopy: ToolPageCopy;
}

export default function PdfToTextPage({ pageCopy }: PdfToTextPageProps) {
  const fileInputCopy = pageCopy.fileInput!;
  const [file, setFile] = useState<File | null>(null);
  const [message, setMessage] = useState("");
  const [downloadUrl, setDownloadUrl] = useState("");
  const [error, setError] = useState("");
  const toTextMutation = usePdfToTextMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await toTextMutation.mutateAsync(file);
      setMessage(result.message);
      setDownloadUrl(getDownloadUrl(result.download_url));
    } catch (err) {
      setError(getApiErrorMessage(err, "PDF to text failed."));
    }
  };
    ...
    <section className="app-shell tool-page-shell relative">
      ...
      <div className="glass-card tool-page-panel">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <button onClick={handleSubmit} disabled={toTextMutation.isPending || !file} className="btn-accent">
            {toTextMutation.isPending ? pageCopy.actions.busyLabel : pageCopy.actions.primaryLabel}
          </button>
        </div>
