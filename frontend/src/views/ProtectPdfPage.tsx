...
import { getApiErrorMessage, useProtectPdfMutation } from "../hooks/usePdfMutations";

interface ProtectPdfPageProps {
  pageCopy: ToolPageCopy;
}

export default function ProtectPdfPage({ pageCopy }: ProtectPdfPageProps) {
  const fileInputCopy = pageCopy.fileInput!;
  const passwordFieldCopy = pageCopy.passwordField!;
  const [file, setFile] = useState<File | null>(null);
  const [password, setPassword] = useState("");
  const [message, setMessage] = useState("");
  const [downloadUrl, setDownloadUrl] = useState("");
  const [error, setError] = useState("");
  const protectMutation = useProtectPdfMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await protectMutation.mutateAsync({ file, password });
      setMessage(result.message);
      setDownloadUrl(getDownloadUrl(result.download_url));
    } catch (err) {
      setError(getApiErrorMessage(err, "Protection failed."));
    }
  };
  ...
  return (
      <section className="app-shell tool-page-shell relative">
        <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
          <div className="orb orb-violet" style={{ width: "280px", height: "280px", top: "-40px", left: "-60px" }} />
        </div>
        ...
        <div className="glass-card tool-page-panel">
          ...
          {/* Password */}
          <div className="mb-6">
            <label className="block text-sm font-semibold mb-2" style={{ color: "var(--text-primary)" }}>{passwordFieldCopy.label}</label>
            <input
              type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              placeholder={passwordFieldCopy.placeholder}
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button onClick={handleSubmit} disabled={protectMutation.isPending || !file || !password} className="btn-accent">
              {protectMutation.isPending ? pageCopy.actions.busyLabel : pageCopy.actions.primaryLabel}
            </button>
          </div>
