...
import { getApiErrorMessage, useCompressPdfMutation } from "../hooks/usePdfMutations";

interface CompressPdfPageProps {
  pageCopy: ToolPageCopy;
}

export default function CompressPdfPage({ pageCopy }: CompressPdfPageProps) {
  const fileInputCopy = pageCopy.fileInput!;
  const [file, setFile] = useState<File | null>(null);
  const [message, setMessage] = useState("");
  const [downloadUrl, setDownloadUrl] = useState("");
  const [error, setError] = useState("");
  const compressMutation = useCompressPdfMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await compressMutation.mutateAsync(file);
      ...
      <section className="app-shell tool-page-shell relative">
        ...
        <div className="glass-card tool-page-panel">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button onClick={handleSubmit} disabled={compressMutation.isPending || !file} className="btn-accent">
              {compressMutation.isPending ? pageCopy.actions.busyLabel : pageCopy.actions.primaryLabel}
            </button>
          </div>
